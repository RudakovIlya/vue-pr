<template>
  <div class="page">
    ...
    <div class="question">
      ...
      <strong>
        Необходимо их объединить в один список, учитывая, что длинны этих
        списков могут отличаться. Результат объединения будет такой:</strong
      >
    ...
    <div class="process">
      ...
      <div>------></div>
      <base-button @click="result = mergedList(list1, list2)"
        >Получить</base-button
      >
      <div>------></div>
      <div class="result">
    ...
    <code class="code">
      ...
      const arr2 = list2.filter(item => !list1.find(innerItem => innerItem.id
      === item.id))<br />
...
export default {
  ...
  methods: {
    mergedList(list1, list2) {
      ...
      const arr2 = list2.filter(
        (item) => !list1.find((innerItem) => innerItem.id === item.id),
      );
      return [...arr1, ...arr2];
    },
  },
